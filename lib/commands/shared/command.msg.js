...
const command_app_1 = require("./command.app");
...
class AppMsgSender {
    constructor(withMention = false, withReply = false, replyChannelId, messageType = types_1.MessageType.kmarkdown) {
        ...
        this.wrongArgs = async (data, resultType = command_types_1.ResultTypes.WRONG_ARGS) => {
         ...
         */
        this.reply = async (content, data, resultType = command_types_1.ResultTypes.SUCCESS) => {
            return this.send(content, data, resultType, {
                reply: true,
                mention: true,
            });
        };
        this.replyWithoutMention = async (content, data, resultType = command_types_1.ResultTypes.SUCCESS) => {
            return this.send(content, data, resultType, {
                reply: true,
                mention: false,
            });
        };
        this.mention = async (content, data, resultType = command_types_1.ResultTypes.SUCCESS) => {
            return this.send(content, data, resultType, {
                reply: false,
                mention: true,
            });
        };
        this.sendOnly = async (content, data, resultType = command_types_1.ResultTypes.SUCCESS) => {
            return this.send(content, data, resultType, {
                reply: false,
                mention: false,
            });
        };
        this.send = async (content, data, resultType = command_types_1.ResultTypes.SUCCESS, sendOptions) => {
            ...
            const msgType = (sendOptions === null || sendOptions === void 0 ? void 0 : sendOptions.msgType) !== undefined
                ? sendOptions.msgType
                : this.messageType;
            const msgSent = bot.sendChannelMessage(msgType, replyChannelId, (withMention ? `${mention_by_id_1.mentionById(data.msg.authorId)} ` : '') + content, (sendOptions === null || sendOptions === void 0 ? void 0 : sendOptions.reply) ? data.msg.msgId : undefined);
            return command_app_1.initFuncResult(data, resultType, msgSent);
        };
        if (replyChannelId)
...
//# sourceMappingURL=command.msg.js.map